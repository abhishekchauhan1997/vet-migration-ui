...
import DynamicOptionList from "react-form-builder2/lib/dynamic-option-list";
import WYSIWYGEditor from "UIComponents/WYSIWYGEditor";

// * utils *
import { get } from "react-form-builder2/lib/stores/requests";
import ID from "react-form-builder2/lib/UUID";
import IntlMessages from "react-form-builder2/lib/language-provider/IntlMessages";
...
export default class FormElementsEdit extends React.Component {
    ...
    componentDidMount() {
        ...
        // for shallow reference bug
        // in custom components
        if (currentElement.custom) {
            if (currentElement.options) {
                currentElement.options = simpleDeepCopy(currentElement.options);
            }

            if (currentElement.props) {
                currentElement.props = simpleDeepCopy(currentElement.props);
            }
        }

        // // setting toolbar according to element
        // const currentElementType =
        //     currentElement["key"] ?? currentElement["element"];

        // let toolbar = defaultToolbar;

        // if (["Header", "Paragraph"].includes(currentElementType)) {
        //     // toolbar changes...
        // }
    ...
    editCustomProp(propName, value) {
        const this_element = this.state.element;
        this_element.props[propName] = value;
    ...
    onEditorStateChange(property, htmlContent) {
    ...
    render() {
        ...
        const showCommentCheckbox = [
            "CheckboxesWithFiles",
            "RadioGroupWithFiles",
            "SelectWithFiles",
        ].includes(this.state.element?.key);
        const showNumberCheckbox = this.state.element?.key === "SimpleInput";

        const canBeNumber = this.props.element?.props?.number;
        const canHaveComments = this.props.element?.props?.canHaveComments;
        ...
        if (this.props.element.hasOwnProperty("label")) {
            editorState = this.props.element.label;
        }
